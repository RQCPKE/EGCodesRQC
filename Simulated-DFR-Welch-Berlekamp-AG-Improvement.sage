...
# interpolate and zero polynomials:  A(g_i)=0;  I(g_i)=y_i
...
def reconstruction(g, y, k, n):
    """
    Using Loidreau's improved technique proposed in DCC 2018 (section 4.4.2) 
    Returns (N1, V1, A, I) where (N1 = P1, V1 = W1)
    P1*A + W1*I = W1*f for correct decoding 
    """
    # Initialization
    A, I = interpolate_vect_and_zero(g, y, k)

    #A = S.minimal_vanishing_polynomial(g[:k])
    #I = S.lagrange_polynomial(list(zip(g[:k], y[:k])))
...
def Welch_Berlekamp_decode(g, y, k, n):
    # reconstruction
    N, V, A, I = reconstruction(g, y, k, n)
    # recover f
    # f = V^{-1}(N*A) + I
    R = N*A
    Q, _ = R.left_quo_rem(V)
    f_rec = Q + I
    return f_rec
