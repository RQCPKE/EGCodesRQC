...
def LowMS_Encap(Public_Key):  
    HH = Public_Key
    e = random_small_vector_genenration(m, N*n, r)   
    e1 = e[:n]; e2 = e[n:2*n]; e3 = e[2*n:3*n]; e4 = e[3*n:4*n]; e5 = e[4*n:5*n]
    s1 = HH*e1; s2 = HH*e2; s3 = HH*e3; s4 = HH*e4; s5 = HH*e5
    C = matrix(N, n-k, [s1, s2, s3, s4, s5]).transpose()
    EE = matrix(N, n, [e1, e2, e3, e4, e5]).transpose()
...
def LowMS_Decap(Private_Key, Ciphertext):  
    ...
    s1 = SC[0]; s2 = SC[1]; s3 = SC[2]; s4 = SC[3]; s5 = SC[4]
    y1 = H.solve_right(s1); y2 = H.solve_right(s2); y3 = H.solve_right(s3)
    y4 = H.solve_right(s4); y5 = H.solve_right(s5)
    r = math.floor((n-k)*N/(N+1))   
    Z = Moore_matrix(g, k + r).transpose()
    A2 = block_diagonal_matrix(Z, Z, Z, Z, Z) 
    ...
    Y5 = Moore_matrix(y5, r+1).transpose()
    A1 = block_matrix(N, 1, [Y1, Y2, Y3, Y4, Y5]) 
    ...
    N4 = OPR(list(N4_vector)); N5 = OPR(list(N5_vector))
    ff1, re = N1.left_quo_rem(-V); ff2, re = N2.left_quo_rem(-V); ff3, re = N3.left_quo_rem(-V)
    ff4, re = N4.left_quo_rem(-V); ff5, re = N5.left_quo_rem(-V)
    ...
    PE = matrix(N, n,[ee1, ee2, ee3, ee4, ee5]).transpose()  # (PE = P*E)
